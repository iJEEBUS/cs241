/**************************************************************************************
 * Lab 6 - Bitwise shift encoding/decoding - cs241 - GVSU
 *
 * A program that will encode/decode a file of unsigned chars into a file of unsigned 
 * long integers (8 chars per 1 integer).
 *
 * @resources:
 * 	encode method written by Professor Wang.  
 *
...
int decode(FILE * infile, FILE * outfile)
{
	...
	while (fscanf(infile, "%ld, ", &ul_holder) != EOF)
	{
		...
		while (n < (char_per_ul - 1))
		{
			...
			ul_holder = ul_holder >> char_per_ul;
			ch_holder = ul_holder;
			n++;
			reverse_count--;
